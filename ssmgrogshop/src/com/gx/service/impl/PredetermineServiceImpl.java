...
public class PredetermineServiceImpl implements PredetermineService {
    ...
    @Override
    public Page<PredeterminePo> pageFuzzyselect(String receiveTeamName, String passengerName, int predetermineStateID,
            Page<PredeterminePo> vo) {
        int start = 0;
        if (vo.getCurrentPage() > 1) {
            start = (vo.getCurrentPage() - 1) * vo.getPageSize();
        }
        List<PredeterminePo> list = predetermineDao.pageFuzzyselect(receiveTeamName, passengerName, predetermineStateID,
                start, vo.getPageSize());
        vo.setResult(list);
